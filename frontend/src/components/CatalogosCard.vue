...
const abrirModalConfirm = (id: number) => {
    idEliminar.value = id;
...
const obtenerNombreColumna = (item: Record<string, any>, key: string) => {
    ...
    if (key === 'animal') {
        const animal = item.animal ?? item.raza?.animal;

        if (typeof animal === 'string') return animal;
        if (typeof animal === 'object') return animal?.nombre;

        return '-';
    }
    if (key === 'raza') return item.raza?.nombre ?? '-';
    if (key === 'user') return item.cliente?.user?.nombre ?? '-';
...
const emitEliminar = () => {
    if(idEliminar.value) {
        emits('eliminar', idEliminar.value);
    }
    confirmAction.value = '';
...
<template>
    <ModalConfirm :action="confirmAction !== '' ? 'delete' : ''" 
    @confirm="emitEliminar"
    @close="confirmAction = ''"
    />
    ...
    <div class="flex items-stretch w-full mb-2 md:mb-0">
        ...
        <div v-else class="flex w-full min-h-[4rem] md:h-16 rounded-xl shadow-sm overflow-hidden transition-all duration-200"
            :class="[
                isVisible ? 'bg-[#fcf7f2] text-[#523013]' : 'bg-[#fcf7f290] text-[#523013b2]',
                clickable ? 'hover:scale-[1.01] hover:cursor-pointer' : ''
            ]"
            @click="clickable ? emits('modalData', item.id) : null">
                    ...
                    <template v-if="c.key === 'opciones'">
                        <div @click.stop class="flex justify-end w-full md:w-auto">
                            <CatalogosCardBotones :isVisible="isVisible" :id="item.id" @editar="emits('editar', item.id)"
                                @modal-confirm="abrirModalConfirm(item.id)"
                                @cambiar-estado="emits('cambiarEstado', item.id)" />
                        </div>
                    </template>
