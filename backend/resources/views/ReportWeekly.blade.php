...
<div class="report-container">
    ...
    <div class="subhead">Periodo {{$periodo['inicio']}} -- {{$periodo['fin']}}</div>
    ...
    <div>
        <h4 class="finance-subtitle">Distribucion general de citas</h4>  
            ...
            <table class="finance-table">
                ...
                @for($i = 0; $i < 6; $i++)
                    <?php
                        $dates = ['lunes','martes','miercoles','jueves','viernes','sabado'];
                        $hasData = isset($TD_citas[$position]) && 
                        \Carbon\Carbon::parse($TD_citas[$position]->fecha)->dayOfWeek == $i+ 1;
                    ?>

                @if($hasData)
                <tr>
                    <td>{{ $dates[$i] }}</td>
                    <td>{{ $TD_citas[$position]->Total_C }}</td>
                </tr>
                <?php $position++; ?>
                @else
                <tr>
    ...
    <div>
        <h4 class="finance-subtitle">Distribucion general de citas canceladas</h4>  
            ...
            <table class="finance-table">
                ...
                @for($i = 0; $i < 6; $i++)
                    <?php
                        $dates = ['lunes','martes','miercoles','jueves','viernes','sabado'];
                        $hasData = isset($cancelada[$position]) && 
                        \Carbon\Carbon::parse($cancelada[$position]->fecha)->dayOfWeek == $i+ 1;
                    ?>

                @if($hasData)
                <tr>
                    <td>{{ $dates[$i] }}</td>
                    <td>{{ $cancelada[$position]->Total_C }}</td>
                </tr>
                <?php $position++; ?>
                @else
                <tr>
    ...
    <div>
        <h4 class="finance-subtitle">Distribucion general de citas siendo atendidas</h4>  
            ...
            <table class="finance-table">
                ...
                @for($i = 0; $i < 6; $i++)
                    <?php
                        $dates = ['lunes','martes','miercoles','jueves','viernes','sabado'];
                        $hasData = isset($en_proceso[$position]) && 
                        \Carbon\Carbon::parse($en_proceso[$position]->fecha)->dayOfWeek == $i+ 1;
                    ?>

                @if($hasData)
                <tr>
                    <td>{{ $dates[$i] }}</td>
                    <td>{{ $en_proceso[$position]->Total_C }}</td>
                </tr>
                <?php $position++; ?>
                @else
                <tr>
    ...
    <div>
        <h4 class="finance-subtitle">Distribucion general de citas completadas</h4>  
            ...
            <table class="finance-table">
                ...
                @for($i = 0; $i < 6; $i++)
                    <?php
                        $dates = ['lunes','martes','miercoles','jueves','viernes','sabado'];
                        $hasData = isset($realizada[$position]) && 
                        \Carbon\Carbon::parse($realizada[$position]->fecha)->dayOfWeek == $i+ 1;
                    ?>

                @if($hasData)
                <tr>
                    <td>{{ $dates[$i] }}</td>
                    <td>{{ $realizada[$position]->Total_C }}</td>
                </tr>
                <?php $position++; ?>
                @else
                <tr>
    ...
    <div class="section">
        <h2> Citas de las mascotas registradas</h2>
        <h4 class="finance-subtitle">Mascotas con cita en esta semana</h4>
        @if(isset($mas_cita) && count($mas_cita) > 0)
            <table class="finance-table">
                <thead>
                <tr>
                    <th> Mascota</th>
        ...
        <h4 class="finance-subtitle">Mascotas con cita  agendada para otra semana</h4>
        @if(isset($mas_cita_otra) && count($mas_cita_otra) > 0)
            <table class="finance-table">
                <thead>
                <tr>
                    <th> Mascota</th>
        ...
        @endif
         ...
         <h4 class="finance-subtitle">Mascotas sin cita agendada</h4>
        @if(isset($mas_sin_cita) && count($mas_sin_cita) > 0)
            <table class="finance-table">
                <thead>
                <tr>
                    <th> Mascota</th>
    ...
    <div class="section">
        ...
        <h4 class="finance-subtitle">Rendimiento diario</h4>  
            <table class="finance-table">
                <thead>
                <tr>
                    <th>Dia</th>
                    <th>Total de citas completadas</th>
                    <th>Total </th>
                </tr>
                </thead>
                <tbody>
    <?php $position = 0; $promedio = 0;?>

    @for($i = 0; $i < 6; $i++)
        <?php
        $dates = ['lunes','martes','miercoles','jueves','viernes','sabado'];
        $hasData = isset($finanzas[$position]) && 
                   \Carbon\Carbon::parse($finanzas[$position]->fecha)->dayOfWeek == $i;
        ?>
        @if($hasData)
        <?php $promedio += $finanzas[$position]->total?>
            <tr>
                <td>{{ $dates[$i] }}</td>
                <td>{{ $finanzas[$position]->total_citas }}</td>
                <td>${{number_format( $finanzas[$position]->total ?? 0 ,2)}}</td>
            </tr>
            <?php $position++; ?>
        @else
            <tr>
    ...
    <?php 
    if(count($finanzas)>0){
        $promedio = $promedio/count($finanzas);
    }else{
        $promedio = 0; 
    }
    ?>
 <table class="finance-table">
        <thead>
            <tr>
                <th>Promedio de ganancias en los dias donde se completaron las citas</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td style="font-weight:bold;">{{number_format($promedio,2)}}</td>
            </tr>
        </tbody>
    </table>
